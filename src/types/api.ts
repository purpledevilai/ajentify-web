...
/* -------------------------------------------------------------------------- */
/* Contexts (read-only inspector)                                             */
/* -------------------------------------------------------------------------- */

/**
 * A single conversation message on a context. The GET /context/{id} endpoint
 * returns a discriminated union: plain messages carry `sender`/`message`,
 * while tool activity (only present when `with_tool_calls=true`) arrives as
 * `tool_call` / `tool_response` entries.
 */
export type ApiContextMessage =
  | { sender: "ai" | "human" | "system"; message: string }
  | {
      type: "tool_call";
      tool_call_id: string;
      tool_name: string;
...
/** Row shape returned by the paginated GET /org-contexts list endpoint. */
export interface ApiOrgContextSummary {
  context_id: string;
  agent_id: string;
  org_id: string;
  user_id: string;
