...
| `/app/documents` | List of all JSON documents in the user's org. Searchable, sortable, click-throughable. | `set_search`, `set_sort`, `create_new` (clicks "+ New document" — creates an untitled document and routes to its detail). |
| `/app/documents/{document_id}` | Edit one document: name, JSON data (via code editor). | `set_name`, `set_data` (pass valid JSON as a string). Saving and deletion are the user's job. |
